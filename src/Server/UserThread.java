...
public class UserThread extends Thread {
    ...
    public boolean login(String nickname, String password) {
        try {
            if (db.login(nickname, password)) {
                sentResponse("806 Successful Login");
                return true;
            }
        } catch(IsLoggedException ile){
            sentResponse("Error 807: User already logged");
        } catch(NonValidPasswordException npe){
            sentResponse("Error 803: Non valid password");
        } catch(InvalidNickException ine){
            sentResponse("Error 801: Non valid nickname");
        }
        return false;
    }
    ...
    public void newFriends(String friend){
        try {
            ...
            sentResponse("810 Friend correctly added");
        }catch(InvalidNickException ine){
            sentResponse("Error 811: This friend didn't join WordQuizzle");
        }catch(FriendshipException fe){
            sentResponse("Error 813: Friendship already added");
        }
    }
    ...
    public void newGame(String friend_nick){
        ...
        if(!(db.lista_amici(nickname)).contains(friend_nick))
            sentResponse("Error 817: "+friend_nick+" is not your friend");
        //controllo che l'utente da sfidare sia online
        else if(!db.userOnline(friend_nick))
            sentResponse("Error 819: "+friend_nick+" is not online");
        //e che non sia impegnato in un'altra sfida
        else if(db.userBusy(friend_nick))
            sentResponse("Error 821: "+friend_nick+" is busy in another game");
        else {
